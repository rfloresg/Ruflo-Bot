// Ruflo Bots: demo sin backend — módulo ES
const state = {
  step: "greeting",
...
function confirmReserva() {
  const r = state.reserva;
  bubble(
    `¡Listo! 🎉 Tu reserva para <b>${r.personas}</b> el <b>${r.fecha}</b> a las <b>${r.hora}</b> a nombre de <b>${r.nombre}</b> ha quedado registrada (DEMO).`
  );
  ...
  // 2) Añade un servicio de email y una plantilla con variables: nombre, fecha, hora, personas
  // 3) Descomenta y reemplaza YOUR_PUBLIC_KEY, YOUR_SERVICE_ID, YOUR_TEMPLATE_ID
